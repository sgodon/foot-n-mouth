...
Data <- function(p = 10, n = 20, sigma2y = 0.2, t = 0.5, mu_beta = 0.5, sd_beta = 0.707, a = 0.5) {
  X_mat <- sim_X(p,n,t)
...
FPR <- rep(0,p+1)
TP <- rep(0,p+1)
FP <- rep(0,p+1)                                                      # Set up vectors for TN,TP, FN, FP, TPR & FPR
TN <- rep(0,p+1)
FN <- rep(0,p+1)
for (i in (1:p+1)) {                                                  # Calculate TPR and FPR for versions of a predicted beta vector where predictions are added to the vector one at a time
  ...
  T <- table(beta_condition, beta_predicted_condition)
  table_df<-data.frame(T)
  M <- apply(as.matrix.noquote(table_df),2,as.numeric)                # Use table function and extract TP etc data
  TN[i] <- M[1,3]
  FN[i] <- M[2,3]
  if (nrow(M)==4) {
    FP[i] <- M[3,3]                                                   # Initially the matrix of extracted data only has two rows so only interogate rows 3&4 when they become established
    TP[i] <- M[4,3]
  }    
 
  TPR[i] <- TP[i]/(TP[i] + FN[i])                                     # calculate TPR and FPR for each iteration
  FPR[i] <- FP[i]/(FP[i] + TN[i])
 if (i<p +1){
   beta_predicted_condition[i] <- ROC_mat[i,4]                        # add value from "binarised" predicted beta to beta_predicted condition
 } 
}
